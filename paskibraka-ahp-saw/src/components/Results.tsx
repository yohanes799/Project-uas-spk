...
const Results: React.FC<ResultsProps> = ({
  ...
  // --- PINTU DARURAT (Cek apakah data ada sebelum menghitung topScore) ---
  const firstRank = sawResult?.rankings?.[0] as any;

  // Ambil nilai dengan aman (Cek preferenceValue dulu, kalau tidak ada pakai nilai_total)
  const topScore = firstRank
    ? (firstRank.preferenceValue ?? firstRank.nilai_total ?? 1)
    : 1;

  // --- CEK DATA ---
  if (!sawResult || !sawResult.rankings || sawResult.rankings.length === 0) {
    return (
      <div className="p-10 text-center text-gray-500">
        Data hasil belum tersedia. Silakan hitung kembali.
      </div>
    );
  }

  // ... sisanya kode tabBtn Anda ...
    ...
    <div className="p-8 flex flex-col gap-6">
      ...
      <div>
        <h2 className="text-2xl font-bold text-gray-900 mb-2">
          Langkah 4: Hasil Hybrid AHP-SAW
        </h2>
        <p className="text-gray-500 text-sm">
          Hasil seleksi peserta paskibraka berdasarkan kombinasi metode AHP
          (pembobotan) dan SAW (perangkingan).
        </p>
      </div>

      {/* Top 3 Podium */}
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {sawResult.rankings.slice(0, 2).map((entry, index) => {
          // TRANSLASI DATA: Memaksa format backend menjadi format frontend
          const rank = (entry as any).rank ?? index + 1; // Jika rank tidak dikirim, pakai urutan index
          const name =
            (entry as any).alternativeName ??
            (entry as any).nama_peserta ??
            'Tanpa Nama';
          const altId =
            (entry as any).alternativeId ??
            (entry as any).id ??
            `card-${index}`;

          const rawVal =
            (entry as any).preferenceValue ?? (entry as any).nilai_total ?? 0;
          const score =
        ...
        ${rank === 1 ? 'bg-gradient-to-br from-yellow-50 to-amber-100 border-yellow-400' : ''}
        ${rank === 2 ? 'bg-gradient-to-br from-slate-50 to-gray-100 border-slate-400' : ''}
        ${rank === 3 ? 'bg-gradient-to-br from-orange-50 to-orange-100 border-orange-300' : ''}
      `}
            >
              <div className="text-4xl mb-2">
                {typeof medalEmoji === 'function' ? medalEmoji(rank) : rank}
              </div>
        ...
        <div className="overflow-x-auto rounded-xl border border-gray-200">
          <table className="w-full text-sm">
            ...
            <tbody>
              {sawResult &&
              sawResult.rankings &&
              sawResult.rankings.length > 0 ? (
                sawResult.rankings.map((entry, index) => {
                  // TRANSLASI DATA YANG SAMA
                  ...
                  const rawVal =
                    (entry as any).preferenceValue ??
                    (entry as any).nilai_total ??
                    0;
                  const score =
                  ...
                  const pct =
                    topScore && topScore > 0 ? (score / topScore) * 100 : 0;
                  ...
                  return (
                    <tr
                      key={altId}
                      className={`${rowBg} hover:brightness-95 transition`}
                    >
                      <td className="px-4 py-3 font-bold text-lg">
                        {rank <= 3
                          ? typeof medalEmoji === 'function'
                            ? medalEmoji(rank)
                            : rank
                          : `#${rank}`}
                      </td>
                      ...
                      <td className="px-4 py-3">
                        {rank <= 3 ? (
                          ...
                          <span className="px-2.5 py-1 text-xs font-bold bg-gray-100 text-gray-500 rounded-full">
                            Cadangan
                          </span>
                        )}
                      </td>
      ...
      {activeTab === 'saw-normalized' && (
        <div className="flex flex-col gap-3">
          <p className="text-xs text-gray-500 bg-gray-50 rounded-lg px-4 py-2 border border-gray-200">
            Normalisasi SAW: benefit = x<sub>ij</sub> / max(x<sub>j</sub>), cost
            = min(x<sub>j</sub>) / x<sub>ij</sub>
          </p>
          <div className="overflow-x-auto rounded-xl border border-gray-200">
            <table className="w-full text-sm">
              <thead className="bg-gray-50 text-xs uppercase text-gray-500 tracking-wide">
                <tr>
                  ...
                  {criteria.map((c) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <span
                          className={`inline-block mt-1 px-1.5 py-0.5 rounded-full text-[10px] font-semibold ${conf?.type === 'benefit' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-600'}`}
                        >
                          {conf?.type}
                        </span>
                      </th>
                  ...
                  <th className="px-3 py-3 text-center font-semibold text-blue-700">
                    Vi
                  </th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-100">
                {sawResult.rankings.map((entry) => (
                  <tr
                    ...
                    className={`hover:bg-gray-50 transition ${entry.rank === 1 ? 'bg-yellow-50' : ''}`}
                  >
                    <td className="px-4 py-3 font-semibold text-gray-800">
                      {entry.rank <= 3 && (
                        <span className="mr-1">{medalEmoji(entry.rank)}</span>
                      )}
                    ...
                    {criteria.map((c) => (
                      <td
                        key={c.id}
                        className="px-3 py-3 text-center font-mono text-xs text-gray-600"
                      >
                        {(
                          sawResult.normalizedMatrix?.[entry.alternativeId]?.[
                            c.id
                          ] ?? 0
                        ).toFixed(7)}
                      </td>
                ...
                <tr className="bg-blue-50 font-bold">
                  ...
                  {criteria.map((c) => (
                    <td
                      key={c.id}
                      className="px-3 py-3 text-center font-mono text-xs text-blue-700"
                    >
        ...
        <div className="flex flex-col gap-4">
                ...
                {criteria.map((rowCrit) => (
                  <tr key={rowCrit.id} className="hover:bg-gray-50 transition">
                    ...
                    {criteria.map((colCrit) => (
                      ...
                      >
                        {(
                          ahpResult.normalizedMatrix[rowCrit.id]?.[
                            colCrit.id
                          ] ?? 0
                        ).toFixed(3)}
                      </td>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {[
              ...
              <div
                key={m.label}
                className="bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 flex flex-col gap-1"
              >
                <span className="text-xs font-semibold text-gray-400 uppercase tracking-wide">
                  {m.label}
                </span>
                <span
                  className={`text-lg font-bold font-mono ${m.label === 'CR' ? (ahpResult.isConsistent ? 'text-green-600' : 'text-amber-600') : 'text-gray-800'}`}
                >
                  {m.value}
                </span>
              </div>
              ...
              {sawResult.rankings.slice(0, 3).map((entry) => (
                ...
                >
                  ...
                  <span className="ml-auto font-mono text-gray-500">
                    {entry.preferenceValue.toFixed(3)}
                  </span>
                </li>
              ))}
            </ol>
              ...
              {criteria.map((c) => (
                <li key={c.id} className="flex items-center gap-2 text-sm">
                  ...
                  <span className="font-bold font-mono text-blue-600">
                    {((ahpResult.weights[c.id] ?? 0) * 100).toFixed(2)}%
                  </span>
                </li>
              ))}
            </ul>
